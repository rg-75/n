...
$idf = "";
$id1 = file_get_contents("id.txt");
$txt = file_get_contents("id_unique_ads.txt");
$id = explode("
",$txt);
// print_r($id);
foreach($id as $a ){
$idf = file_get_contents("id.txt");
$exf = explode("\n",$idf);
	$b = str_replace("
","",$a);
	// echo $b;
if(!in_array($b,$exf)){
$id1 .= $b."\n";
$myfileid = fopen("id.txt", "w") or die("Unable to open file!");
fwrite($myfileid,$id1);
// echo " zaki";
$ch = curl_init();
...
foreach($ex as $v){
	...
	if(isset($ex2[1]) 
		...
		$nf = file_get_contents("n.txt");
		$exnf = explode("\n",$nf);
		if(!in_array($ex2[1],$exnf)){
			$n .= $ex2[1]."\n";
